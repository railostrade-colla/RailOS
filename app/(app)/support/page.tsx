...
// Status → Badge color
const STATUS_META: Record<SupportTicket["status"], { labelKey: string; color: "green" | "yellow" | "neutral" }> = {
  open:     { labelKey: "stOpen",     color: "yellow" },
  answered: { labelKey: "stAnswered", color: "green" },
  closed:   { labelKey: "stClosed",   color: "neutral" },
}

const PRIORITY_META: Record<SupportTicket["priority"], { labelKey: string; color: "green" | "yellow" | "red" }> = {
  low:    { labelKey: "prLow",    color: "green" },
  medium: { labelKey: "prMedium", color: "yellow" },
  high:   { labelKey: "prHigh",   color: "red" },
}

// FAQ category (Arabic, lib-canonical for matching) → i18n key.
const CAT_KEY: Record<string, string> = {
  "عام": "catGeneral", "الاستثمار": "catInvesting", "المحفظة": "catPortfolio",
  "العقود": "catContracts", "التوثيق": "catKyc", "الرسوم": "catFees", "الأمان": "catSecurity",
}
  ...
  const handleSubmitTicket = async () => {
    ...
    if (!result.success) {
      const map: Record<string, string> = {
        ...
        invalid_input: t("fillSubjectMsg"),
        missing_table: t("tablesNotPub"),
        rls: t("noTicketPermission"),
      }
      showError(map[result.reason ?? ""] ?? t("sendFailed"))
      setSubmitting(false)
      return
    }
    showSuccess(t("ticketSent"))
...
<div className="relative z-10 px-3 lg:px-8 py-6 lg:py-12 max-w-4xl mx-auto pb-20">
          ...
          <Card variant="gradient" color="blue" className="mb-7">
            <div className="text-center mb-4">
              ...
              <h2 className="text-base font-bold text-white mb-1">{t("heroTitle")}</h2>
              <p className="text-[11px] text-neutral-400">{t("heroSubtitle")}</p>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5 justify-center">
              {[
                { labelKey: "ql1", q: "كيف أبدأ؟" },
                { labelKey: "ql2", q: "حساب الأرباح" },
                { labelKey: "ql3", q: "التوثيق" },
                { labelKey: "ql4", q: "العمولات" },
              ].map((ql) => (
                <button
                  key={ql.labelKey}
                  onClick={() => setSearch(ql.q)}
          ...
          <div className="grid grid-cols-3 gap-2 mb-7">
            <button
              onClick={() => showInfo(t("liveChatSoon"))}
            ...
            >
              ...
              <div className="text-[11px] font-bold text-white">{t("liveChat")}</div>
            </button>
            <button
            ...
            >
              ...
              <div className="text-[11px] font-bold text-white">{t("sendTicket")}</div>
            </button>
            <button
              onClick={() => showInfo(t("callInfo"))}
              className="bg-white/[0.05] border border-white/[0.08] hover:bg-white/[0.08] rounded-xl py-3 px-2 flex flex-col items-center gap-2 transition-colors"
            >
              ...
              <div className="text-[11px] font-bold text-white">{t("callUs")}</div>
            </button>
          </div>
          ...
          <div className="mb-7">
            <SectionHeader
              title={t("faqTitle")}
              subtitle={t("faqCount", { n: filteredFAQs.length })}
            />

            <div className="mb-4 -mx-1 px-1 overflow-x-auto">
              <Tabs
                tabs={[
                  { id: "all", icon: "✨", label: t("catAll"), count: tabCounts.all },
                  ...FAQ_CATEGORIES.map((c) => ({ id: c, label: t(CAT_KEY[c] ?? "catGeneral"), count: tabCounts[c] })),
                ]}
                activeTab={tab}
            ...
            {filteredFAQs.length === 0 ? (
              <EmptyState
                ...
                action={{ label: t("sendTicket"), onClick: () => setShowNewTicket(true) }}
                size="md"
              />
            ) : (
                    ...
                    <Card key={faq.id} padding="md" className="!p-0 overflow-hidden">
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                            <Badge color="neutral" variant="soft" size="xs">{t(CAT_KEY[faq.category] ?? "catGeneral")}</Badge>
                          </div>
                          <div className="text-sm font-bold text-white text-right">{t(`q_${faq.id}`)}</div>
                        </div>
                      ...
                      {isOpen && (
                        <div className="px-4 pb-4 -mt-1 border-t border-white/[0.04] pt-3">
                          <p className="text-xs text-neutral-300 leading-relaxed mb-3">{t(`a_${faq.id}`)}</p>
                          <div className="flex items-center justify-between gap-2 flex-wrap">
                            <div className="flex gap-2">
                              <button
                                onClick={() => showSuccess(t("thanksFeedback"))}
                              ...
                              <button
                                onClick={() => showInfo(t("willImprove"))}
                            ...
                            {faq.helpful_count !== undefined && (
                              <span className="text-[10px] text-neutral-500">
                                {t("helpedCount", { n: faq.helpful_count.toLocaleString("en-US") })}
                              </span>
                            )}
                          </div>
                        </div>
          ...
          {tickets.length > 0 && (
            <div className="mb-7">
              <SectionHeader
                title={t("myTickets")}
                subtitle={t("ticketsCount", { n: tickets.length })}
                action={{ label: t("newRequest"), onClick: () => setShowNewTicket(true) }}
              />
              <div className="space-y-2">
                {tickets.map((tk) => {
                  const status = STATUS_META[tk.status]
                  const priority = PRIORITY_META[tk.priority]
                  return (
                    <Card key={tk.id} onClick={() => setTicketModal(tk)} className="flex items-center justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-bold text-white mb-1 truncate">{tk.subject}</div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <Badge color={status.color} variant="soft" size="xs">{t(status.labelKey)}</Badge>
                          <Badge color={priority.color} variant="soft" size="xs">{t(priority.labelKey)}</Badge>
      ...
      >
        <div className="space-y-3">
          <div>
            <div className="text-[11px] text-neutral-400 mb-1.5 font-bold">{t("subjectLabel")}</div>
            <input
          ...
          <div>
            <div className="text-[11px] text-neutral-400 mb-1.5 font-bold">{t("messageLabel")}</div>
            <textarea
      ...
      {ticketModal && (
        <Modal
          ...
          subtitle={t("createdIn", { date: ticketModal.created_at })}
          size="md"
        >
          <div className="flex items-center gap-1.5 mb-4 flex-wrap">
            <Badge color={STATUS_META[ticketModal.status].color} variant="soft">{t(STATUS_META[ticketModal.status].labelKey)}</Badge>
            <Badge color={PRIORITY_META[ticketModal.priority].color} variant="soft">{t("priorityPrefix", { p: t(PRIORITY_META[ticketModal.priority].labelKey) })}</Badge>
          </div>
          <div className="bg-white/[0.04] border border-white/[0.06] rounded-lg p-3 mb-3">
            <div className="text-[10px] text-neutral-500 mb-1">{t("lastUpdate")}</div>
